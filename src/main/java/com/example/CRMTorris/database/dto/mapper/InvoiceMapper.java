...
import com.example.CRMTorris.database.dto.InvoiceDto;
...
public class InvoiceMapper extends AbstractMapper<Invoice, InvoiceDto> {
    ...
    public void setMapper() {
        mapper.createTypeMap(Invoice.class, InvoiceDto.class)
                .addMappings(m -> m.skip(InvoiceDto::setOrderId)).setPostConverter(toDtoConverter());
        mapper.createTypeMap(Invoice.class, InvoiceDto.class)
                .addMappings(m -> m.skip(InvoiceDto::setCompanyId)).setPostConverter(toDtoConverter());
        mapper.createTypeMap(InvoiceDto.class, Invoice.class)
                .addMappings(m -> m.skip(Invoice::setOrder)).setPostConverter(toEntityConverter());
        mapper.createTypeMap(InvoiceDto.class, Invoice.class)
                .addMappings(m -> m.skip(Invoice::setCompany)).setPostConverter(toEntityConverter());
    }
